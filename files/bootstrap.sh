...
CONFIG_FILE="./graphdb/config/d3fend/config.ttl"
...
if ! docker info > /dev/null 2>&1; then
  warn "Docker check failed. Output:"
  docker info || true
  error "Docker is not running or you lack permissions. Please start Docker or add your user to the docker group."
fi
...
if [ ! -f "$CONFIG_FILE" ]; then
  warn "Config file not found at $CONFIG_FILE. Generating default config..."
...
if [ "$EXISTING" -gt 0 ]; then
  ...
  HTTP_CODE=$(curl -s -o /tmp/gdb_create.log -w "%{http_code}" -X POST "$GRAPHDB_URL/rest/repositories" \
  ...
  if [[ "$HTTP_CODE" =~ ^2 ]]; then
    ...
    cat /tmp/gdb_create.log
    echo ""
    error "Failed to create repository (HTTP $HTTP_CODE)."
  fi
fi
...
RESPONSE=$(curl -sf -X POST \
  ...
  }' > /dev/null || error "Failed to trigger import. Check if the file is visible to GraphDB (volume mapping)."
